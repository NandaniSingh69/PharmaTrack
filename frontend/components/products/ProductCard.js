...
function ProductCard({ item, openModal }) {
  return (
    <div className='flex h-full border border-gray-200 shadow-md rounded-md'>
      <div className='h-full w-4/12 bg-pharmaGreen-600 rounded-l-md flex items-center justify-center'>
        {/* use native img to avoid next image host config for now */}
        <img src={item.itemImage || '/logo4.png'} alt={item.name} className='h-32 object-contain' />
      </div>
      <div className='flex w-3/4 p-4'>
        <div className='w-1/2 space-y-3'>
          <p className='text-sm font-semibold'>{item.name}</p>
          <p className='text-xs font-thin'>{item.barcodeId}</p>
          ...
          <p className='text-xs font-thin'>Qty: {item.quantity}</p>
        </div>
