...
function AddProduct() {
  ...
  const saveItem = async (item) => {
    try {
      ...
      const manufacturedEpoch = getCustomDateEpoch(item.manufacturedDate);
      const expiryDays = Number(item.expiringDate);

      item.manufacturedDate = manufacturedEpoch;
      item.expiringDate = manufacturedEpoch + (expiryDays * 24 * 60 * 60);
      ...
      console.log("Register new drug batch:", (end - start).toFixed(2), "ms");
        ...
        <div className='w-full'>
          ...
          <form onSubmit={handleSubmit(saveItem)}>
            ...
            <div className='mb-4'>
              ...
              <input {...register("expiringDate", { required: true, valueAsNumber: true })} className='w-full h-9 rounded-md p-2 text-sm border' type="number" />
