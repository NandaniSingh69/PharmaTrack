...
function TimelineItem({icon, title, roleLabel, name, email, accountId, ts}) {
  return (
    <div className="flex gap-4 items-start bg-white p-4 rounded-lg shadow-md">
      <div className="flex-shrink-0 mt-1 text-pharmaGreen-700">
        {icon}
      </div>
      <div>
        <div className="flex items-center justify-between">
          <h4 className="text-lg font-semibold">{title || name}</h4>
          {roleLabel && <span className="text-xs uppercase text-gray-500 ml-4">{roleLabel}</span>}
        </div>
        <p className="text-sm text-gray-600">{email}</p>
        <p className="text-xs text-gray-400 mt-1">{firstAndLastFour(accountId)}</p>
        {ts && <p className="text-xs text-gray-400 mt-2">{new Date(Number(ts) * 1000).toLocaleString()}</p>}
      </div>
    </div>
  );
}
...
export default function ItemDetails() {
  ...
  const fetchAccountDetails = async (address, setter) => {
    try {
      ...
      // details matches Types.AccountDetails
      setter(details);
    } catch (err) {
      // ignore missing parties
    ...
    <div className="p-6 md:w-10/12 md:mx-auto">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-5/12 bg-white p-6 rounded-lg shadow">
          <img src={item?.itemImage || "/logo3.png"} alt={item?.name} className="w-28 h-28 object-cover rounded-md mb-4"/>
          <h2 className="text-2xl font-bold">{item?.name}</h2>
          <p className="text-sm text-gray-600 mt-3">MFG: {item?.manufacturedDate}</p>
          <p className="text-sm text-gray-600">EXP: {item?.expiringDate}</p>
          <p className="text-sm text-gray-600 mt-2">Barcode: {item?.barcodeId}</p>
